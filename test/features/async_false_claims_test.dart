...
void main() {
  testWidgets('the hero does not claim you have no prayers when it failed',
    ...
    // findsWidgets, not findsOneWidget: an empty account now gets three
    // slides — the prayer one plus invitations to add a promise and to start
    // reading — instead of one, and a carousel of more than one slide pads
    // itself with a copy of the first and last so it can scroll both ways.
    // The count is the clone; the claim being asserted is unchanged.
    expect(find.text("Your prayers couldn't be loaded"), findsWidgets);
  ...
  testWidgets('and still says so when there genuinely are none',
    ...
    // itself with a copy of the first and last so it can scroll both ways.
    // The count is the clone; the claim being asserted is unchanged.
    expect(find.text('No active prayers'), findsWidgets);
  });
